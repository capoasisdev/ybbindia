...
export const getCertificateOverview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<CertificateOverview> => {
    ...
    let enrolment = await loadActiveEnrolment(supabase, userId);

    // Staff/admins may not have an enrolment row — synthesise one from the
    // first published course so they can see their certificate checklist.
    if (!enrolment) {
      const { data: rolesRes } = await supabase
        .from("user_roles")
        .select("role")
        .eq("user_id", userId);
      const roles = (rolesRes ?? []).map((r: { role: string }) => r.role);
      const isStaff = roles.some((r: string) =>
        ["super_admin", "content_admin", "reviewer", "support_admin"].includes(r),
      );

      if (isStaff) {
        // Find any course to use as the reference
        ...
        if (firstCourse) {
          enrolment = { id: "staff-synthetic", course_id: firstCourse.id };
        }
      }
    }
  ...
  .handler(async ({ context }): Promise<CertificateCandidate[]> => {
    ...
    const [{ data: profiles }, { data: certs }] = await Promise.all([
      supabase
        ...
        .select("id, full_name, certificate_name, email")
        .in("id", userIds),
