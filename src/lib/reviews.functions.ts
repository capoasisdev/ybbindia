...
export type ReviewDecision = "under_review" | "approved" | "rejected" | "resubmission_required";
...
export const listAdminSubmissions = createServerFn({ method: "GET" })
  ...
  .inputValidator(
    (input?: { status?: string; search?: string; latestOnly?: boolean }) => input ?? {},
  )
  .handler(
    async ({
      data,
      context,
    }): Promise<{ submissions: AdminSubmission[]; counts: Record<string, number> }> => {
      ...
      const [assignmentsRes, learnersRes, reviewsRes, allStatusRes] = await Promise.all([
        assignmentIds.length
          ? supabase
              .from("assignments")
              .select("id, title, is_final_project")
              .in("id", assignmentIds)
      ...
      const assignments = new Map((assignmentsRes.data ?? []).map((a: any) => [a.id, a]));
