...
export async function computeEligibility(
  ...
  const [{ data: attempts }, { data: submissions }, { data: modules }] = await Promise.all([
    supabase
      .from("exam_attempts")
      .select("is_passed")
      .eq("user_id", userId)
  ...
  const examPassed = (attempts ?? []).some((a: { is_passed: boolean | null }) => a.is_passed);
  ...
  const assignmentsApproved = compulsory.every((a: { id: string }) => approvedIds.has(a.id));
  ...
  const lessonsComplete = lessonIds.length > 0 && completed >= lessonIds.length;
