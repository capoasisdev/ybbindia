...
import { ArrowDown, ArrowUp, Loader2, Plus, Trash2, HelpCircle, FileCheck2, BookOpen } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/admin/lessons")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Manage course content: modules, lessons, assignments, and exam questions." },
...
function Page() {
  ...
  const run = async (key: string, queryKey: string, fn: () => Promise<unknown>, message: string) => {
    setBusy(key);
    try {
      await fn();
  ...
  return (
    <AppShell title="Course content admin">
      ...
      <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
        {content?.courseTitle ?? "Course"} — manage modules, lessons, compulsory assignments, and the exam question bank.
      </p>

      <div className="mt-8 space-y-6">
        {/* Lessons Management */}
        {activeTab === "lessons" && <div className="space-y-5">
                ...
                <div className="space-y-3 border-b border-border bg-secondary/40 px-6 py-5">
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setDraftLesson({ ...draftLesson, videoUrl: e.target.value })}
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      onChange={(e) => setDraftLesson({ ...draftLesson, summary: e.target.value })}
                      rows={2}
                    />
                  </div>
                  <div className="flex gap-2">
                    ...
                    >
                      {busy === "create-lesson" ? <Loader2 className="size-4 animate-spin" /> : null}
                      Create lesson
                    </Button>
                          ...
                          <Button
                            ...
                            onClick={() =>
                              run(
                                lesson.id,
                                "admin-course-content",
                                () => moveLesson({ data: { lessonId: lesson.id, direction: "down" } }),
                                "Order updated",
                              )
                            }
                          >
        ...
        {activeTab === "assignments" && <div className="space-y-4">
              ...
              <div className="grid gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setDraftAssignment({ ...draftAssignment, title: e.target.value })}
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    onChange={(e) => setDraftAssignment({ ...draftAssignment, description: e.target.value })}
                ...
                <div className="flex flex-wrap gap-6 items-center">
                  <div className="flex items-center gap-2">
                    <Switch
                      ...
                      onCheckedChange={(c) => setDraftAssignment({ ...draftAssignment, isCompulsory: c })}
                    />
                  ...
                  <div className="flex items-center gap-2">
                    <Switch
                      ...
                      onCheckedChange={(c) => setDraftAssignment({ ...draftAssignment, isPublished: c })}
                    />
                    ...
                    onClick={() =>
                      run(
                        ...
                        async () => {
                          ...
                          setDraftAssignment({ title: "", description: "", isCompulsory: true, isPublished: true });
          ...
          <div className="rounded-2xl border border-border bg-card">
            {isAssignmentsLoading ? (
              <div className="p-6 text-sm text-muted-foreground flex gap-2"><Loader2 className="size-4 animate-spin"/> Loading assignments...</div>
            ) : !assignments || assignments.length === 0 ? (
              <div className="p-10 text-center text-sm text-muted-foreground">No assignments configured yet.</div>
            ) : (
              <ul className="divide-y divide-border">
                    ...
                    {editingAssignment?.id === assignment.id ? (
                      <div className="space-y-4">
                        ...
                        <div className="flex gap-4">
                          ...
                          <Button size="sm" variant="ghost" onClick={() => setEditingAssignment(null)}>
                            Cancel
                          </Button>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-4">
                        <div className="space-y-1 max-w-2xl">
                          ...
                          <p className="text-sm text-muted-foreground whitespace-pre-wrap">{assignment.description ?? "No description provided."}</p>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <span className="text-xs text-muted-foreground">{assignment.isPublished ? "Published" : "Draft"}</span>
                          </div>
                            ...
                            <Switch
                              ...
                              onCheckedChange={(checked) =>
                                run(
                                  ...
                                  () =>
                                    editAssignment({
                                      data: { assignmentId: assignment.id, isCompulsory: checked },
                                    }),
        ...
        {/* Question Bank */}
        {activeTab === "questions" && <div className="space-y-4">
          <div className="flex items-center justify-between">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Textarea
                    ...
                    onChange={(e) => setDraftQuestion({ ...draftQuestion, prompt: e.target.value })}
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setDraftQuestion({ ...draftQuestion, correctOption: e.target.value })}
                    >
                      <option value="">Select the correct option</option>
                      {draftQuestion.options.map((opt, idx) => (
                        <option key={idx} value={opt} disabled={!opt.trim()}>
                          {opt.trim() ? `Option ${idx + 1}: ${opt.trim()}` : `Option ${idx + 1} (Empty)`}
                        </option>
                      ))}
                    </select>
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setDraftQuestion({ ...draftQuestion, marks: parseInt(e.target.value) || 1 })}
                    />
                  </div>
                </div>
                <div className="flex gap-2">
                    ...
                    onClick={() =>
                      run(
                        ...
                        async () => {
                          ...
                          setDraftQuestion({ prompt: "", options: ["", "", "", ""], correctOption: "", marks: 1 });
          ...
          <div className="rounded-2xl border border-border bg-card">
            {isQuestionsLoading ? (
              <div className="p-6 text-sm text-muted-foreground flex gap-2"><Loader2 className="size-4 animate-spin"/> Loading question bank...</div>
            ) : !questions || questions.length === 0 ? (
              <div className="p-10 text-center text-sm text-muted-foreground">No questions configured yet.</div>
            ) : (
              <ul className="divide-y divide-border">
                          ...
                          <div className="grid gap-2 pl-4 sm:grid-cols-2 mt-2">
                            {question.options.map((opt, idx) => {
                              ...
                              return (
                                ...
                                >
                                  <span>{idx + 1}. {opt}</span>
                                  {isCorrect && <span className="font-semibold text-2xs uppercase">Correct</span>}
                                </div>
                              );
                            })}
                          </div>
