      ...
      <div className="max-w-3xl space-y-8">
        ...
        {result && (
          <section
            className={`rounded-2xl border p-6 ${
              result.isPassed ? "border-primary/40 bg-primary/5" : "border-destructive/40 bg-destructive/5"
            }`}
          >
        ...
        <section className="rounded-2xl border border-border bg-card p-6 shadow-soft">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {data.activeAttempt ? (
              <Button onClick={() => resume.mutate(data.activeAttempt!.id)} disabled={resume.isPending}>
                {resume.isPending ? <Loader2 className="size-4 animate-spin" /> : <Clock className="size-4" />}
      ...
      <header className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-border bg-card p-4">
        ...
        <span
          className={`rounded-lg border px-3 py-1.5 font-mono text-sm ${
            lowTime ? "border-destructive/50 text-destructive" : "border-border text-muted-foreground"
          }`}
        >
          {formatClock(msLeft)}
        </span>
