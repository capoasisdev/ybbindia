      ...
      <AppShell title="Assignments">
        ...
        <div className="mt-8 rounded-2xl border border-dashed border-border bg-card p-10 text-center">
          ...
          <Button asChild className="mt-5">
            <Link to="/enrol">View enrolment</Link>
          </Button>
        </div>
      </AppShell>
    );
  ...
  const upload = useMutation({
    mutationFn: async () => {
      ...
      if (!assignment.allowedFileTypes.includes(extension)) {
        throw new Error(`Only ${assignment.allowedFileTypes.join(", ").toUpperCase()} files are accepted`);
      }
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          {assignment.isFinalProject && (
            <p className="text-xs font-medium uppercase tracking-wide text-primary">Final project</p>
          )}
        ...
        <ul className="mt-4 space-y-2 border-t border-border pt-4">
          {assignment.submissions.map((s) => (
            <li key={s.id} className="rounded-lg bg-muted/40 p-3 text-sm">
              ...
              {s.learnerNote && <p className="mt-1 text-xs text-muted-foreground">{s.learnerNote}</p>}
              {s.reviewerFeedback && (
                <p className="mt-2 rounded-md border-l-2 border-primary bg-background p-2 text-xs">
                  ...
                  {s.score !== null && <span className="ml-1 text-muted-foreground">({s.score})</span>}
                </p>
              )}
            </li>
          ))}
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1 text-xs ${
        approved ? "border-primary/40 bg-primary/10 text-primary" : "border-border text-muted-foreground"
      }`}
    >
